...
class TFminiSensor : public PollingComponent, public sensor::Sensor, public UARTDevice{
	...
	void update() override {
		ESP_LOGD("debug", "update");
		if( available() > 0 ) {
			ESP_LOGD("debug", "update");
			int readVal=read();
			ESP_LOGD("debug", "got readVal: %s", readVal);
			if(readVal == HEADER){;//Read while start packet
				uart_value[0] = HEADER;
				ESP_LOGD("debug", "to read");
			...
			//	  flush();
				...
				if (uart_value[8] == (check & 0xff)){ //verify the received data as per protocol
					...
					ESP_LOGD("debug", "got Distance: %s", distance);
